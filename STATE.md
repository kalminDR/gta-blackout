...
Generated **2026-09-10 18:18 UTC** by `state.py`, from the repository itself. Nothing here is written from memory. If it disagrees with any other document, this file is right and the other document is stale.

## Where we are

- **10 sources working**, 0 failing, 2 waiting on a key
- **1 planned sources have no code at all**: chicago
- **211 snapshots** over 201.1 hours (8.4 days); last one 0.0 h ago

## Sources

Newest snapshot: `data/2026-09-10/1818.json`
...
- First: `2026-09-02 09:14 UTC` · Last: `2026-09-10 18:18 UTC`
- 211 of ~202 expected hourly readings
...
|---|---|---|---|
| 🔴 | `bestbuy_*` (6 metrics) | 0/211 | never returned a number — source not authenticating |
| 🔴 | `power_*` (3 metrics) | 0/211 | never returned a number — source not authenticating |
| 🔴 | `baseline_quality` | 0/211 | never returned a number |
| 🔴 | `deviations` | 0/211 | never returned a number |
| 🔴 | `listings_uk_ps5_pro` | 107/211 | frozen at 100 for all 24 readings |
| 🔴 | `listings_uk_xbox_series_x` | 107/211 | frozen at 100 for all 24 readings |
| 🔴 | `listings_us_ps5` | 107/211 | frozen at 100 for all 24 readings |
| 🔴 | `listings_us_ps5_pro` | 107/211 | frozen at 100 for all 24 readings |
| 🔴 | `listings_us_xbox_series_x` | 107/211 | frozen at 100 for all 24 readings |
| 🔴 | `polymarket_market_count` | 171/211 | frozen at 6 for all 24 readings |
| 🔴 | `steam_rank_gta5` | 204/211 | frozen at 6 for all 23 readings |
| 🔴 | `traffic_warsaw_delay_pct` | 199/211 | zero in 10 of 24 readings |
| 🔴 | `yt_rockstar_views_per_hour` | 195/211 | zero in 21 of 24 readings |
| 🔴 | `yt_subscribers` | 207/211 | frozen at 1.37e+07 for all 24 readings |
| 🔴 | `yt_subscribers_per_hour` | 195/211 | frozen at 0 for all 24 readings; zero in 24 of 24 readings |
| 🟠 | `power_es_lag_hours` | 42/211 | missing 5 of 19 since it started |
| 🟠 | `power_es_load_mw` | 42/211 | missing 5 of 19 since it started |
| 🟠 | `power_it_lag_hours` | 42/211 | missing 6 of 21 since it started |
| 🟠 | `power_it_load_mw` | 42/211 | missing 6 of 21 since it started |

**Repaired.** These were failing earlier in the record and are clean across the last 24 readings. Listed so the fix is visible, and so nobody fixes it twice: `power_de_lag_hours`, `power_de_load_mw`, `power_fr_lag_hours`, `power_fr_load_mw`, `power_hu_lag_hours`, `power_hu_load_mw`, `power_nl_lag_hours`, `power_nl_load_mw`, `power_pl_lag_hours`, `power_pl_load_mw`, `power_se_lag_hours`, `power_se_load_mw`, `traffic_budapest_delay_pct`, `traffic_london_delay_pct`, `traffic_losangeles_delay_pct`, `traffic_losangeles_seconds_measured`, `traffic_losangeles_travel_index`.
